...
export class ProductDetailsComponent implements OnInit {
  ...
  newForm(){
    this.refreshForm()
    this.slides = []
    var img = { src: "https://ifapools-alrashidpools.s3.amazonaws.com/images/imgNaN.png" }
        ...
        if(resp.recordset.length === 0) {
          ...
          this.productService.getProductDocuments(res.recordset[0].PCODE,'DOC').subscribe((respo: any) => {
            ...
            for(let i=0; i<respo.recordset.length; i++) {
              const docUrl = "https://ifapools-alrashidpools.s3.amazonaws.com/documents/" + respo.recordset[i].DOCUMENTNAME
              const document = new FormGroup({
          ...
          this.productService.getProductDocuments(res.recordset[0].PCODE,'IMG').subscribe((respo: any) => {
            ...
            if (respo.recordset.length === 0) {
              ...
              for(let i=0; i<respo.recordset.length; i++) {
                const docUrl = "https://ifapools-alrashidpools.s3.amazonaws.com/images/" + respo.recordset[i].DOCUMENTNAME
                const image = new FormGroup({
